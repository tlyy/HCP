...
from bbf.weight_recyclers import get_intermediates, jit_rsp, log_stats
...
class BBFAgent(dqn_agent.JaxDQNAgent):
  ...
  def _training_step_update(self, step_index, offline=False):
    ...
    # intermediates, target_intermediates = self.log_dead_neuron(self.replay_elements['state'][:, 0])
    # log_dict_neurons = (
    #     self.weight_recycler.maybe_log_deadneurons(self.training_steps, intermediates))
    # self._log_stats(log_dict_neurons)
    ...
    if self.hcp:
      ...
      # log_stats(intermediates, self.dead_neuron_threshold, self.base_dir)
      (
        self.online_params,
        self.optimizer_state,
        self.current_count,
